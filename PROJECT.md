...
│   │   │   │   ├── coordination.service.ts # Visit coordination relay: init after accept, send reminders, work-completion checks, confirmVisit con parseDateTimeNatural (AUT-248) y GPS opcional cuando userLatitude es null (AUT-315), notifyProfessionalVisitConfirmed retorna mensaje unificado (nombre cliente, día/hora, dirección, teléfono, link Google Maps) para ser usado como response.text del profesional; ya no envía directamente por WhatsApp — AUT-368. sendRequestMedia deprecado: el envío de fotos/audio en "Ver detalles" ahora lo maneja webhooks.routes.ts via mediaFirst (AUT-290). notifyWorkFinished simplificado: mensaje de rating 1-5 sin botones, step inicial FEEDBACK_RATING (AUT-318)
