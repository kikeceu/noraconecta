...
│   │   │           ├── ProfessionalOrders.tsx     # History: terminal orders, StatCards (text-4xl JetBrains Mono, min-h-[100px], uppercase tracking-wide labels), filters, search, table desktop + mobile cards, RatingDetailModal sin ejes individuales (solo promedio general + comentario y ¿volvería a atenderlo? + comentario), rate user (RateUserModal 2 steps: rating general + ¿volvería a atenderlo? + comentario, AUT-320), space-y-5, max-w-5xl (AUT-156, AUT-179, AUT-181, AUT-183, AUT-320, AUT-344)
...
│   │   │           └── ProfessionalReputation.tsx # Donut chart (160x160, r=68), compliance metrics (text-4xl), metric cards min-h-[120px], completed/rejected/notFulfilled rows text-2xl, recomendación %, tips, sin desglose por eje falso, max-w-5xl (AUT-181, AUT-183, AUT-344)
