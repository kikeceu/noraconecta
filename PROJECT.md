...
│   │   │   ├── date-utils.ts          # ParseDateTimeResult type + parseExactDate (DD/MM HH) + parseDateTimeNatural (lenguaje natural con LLM, discriminated union con reason 'past'|'ambiguous', AUT-237, AUT-248). Prompt incluye próximos 8 días con fechas exactas para evitar bad parse del LLM en días de semana relativos — AUT-475. acepta promptTemplate: string como parámetro obtenido de PromptService (AUT-494) + getDayArgentina, getHoursArgentina, getMinutesArgentina, formatDateTimeArgentina (formato completo: "miércoles 10 de junio a las 10:00", AUT-166, AUT-167, AUT-247)
...
│   │   │   └── professionals/
│   │   │       ├── professionals.routes.ts     # 21 endpoints under /professionals. PATCH /session/:token/profile — AUT-356. PATCH /session/:token/profile/photo — AUT-447. GET /session/:token/departments — AUT-356. POST /admin-create — AUT-351, AUT-354. PATCH /:id (adminUpdate) — AUT-355. PATCH /:id/license-status — AUT-324. GET /session/:sessionToken/earnings — AUT-327. GET /session/:sessionToken/membership-discount — AUT-329. GET /:id/current-plan — AUT-348. DELETE /:id/membership — AUT-353. GET /:id/public-profile — AUT-449. GET /data-changes/professionals: profesionales con cambios pendientes — AUT-359
│   │   │       ├── professionals.controller.ts # Request validation, response formatting. updateProfile: edición de perfil autenticada por session token — AUT-356. uploadProfilePhoto: upload de foto de perfil con session token — AUT-447. getSessionDepartments: departamentos disponibles para edición — AUT-356. adminCreate: alta directa de profesional con disponibilidad opcional — AUT-351, AUT-354. adminUpdate: edición completa con zonas, disponibilidad y archivos — AUT-355. updateLicenseStatus — AUT-324. getEarnings: endpoint de ganancias — AUT-327. getMembershipDiscount — AUT-329. verify soporta ?mode=license para re-subida de credencial — AUT-335. getCurrentPlan: endpoint público de plan activo — AUT-348. cancelMembership: cancelación de membresía activa — AUT-353. getPublicProfile: perfil público del profesional (sin auth) — AUT-449. getProfessionalsWithPendingChanges: IDs de profesionales con cambios sensibles pendientes — AUT-359
...
│   │   │   │   └── memberships.repository.ts # Prisma queries for Membership/Professional models + findExpiringMemberships, markReminderSent, clearExpiredReminderFlags (AUT-330). updateStatus extendido con CANCELED — AUT-353
   ...
   │   │   │   │   ├── config.service.ts    # Key-value config get/update
    ...
    │   │   │   ├── whatsapp/                         # (AUT-497)
    │   │   │   │   ├── whatsapp.service.ts   # WhatsApp usage handler registration (fire-and-forget vía registerTemplateUsageHandler/registerServiceConversationHandler), getCosts: aggregated costs by template + service conversations + avg cost per request + daily trend, getTemplateCatalog, updateTemplate con cache. registerHandlers recibe categoryPrices (Record<string,number>) con precios por categoría desde SystemConfig; el costUsd de cada envío se calcula con _categoryPrices[category] — AUT-497, AUT-501
    │   │   │   │   └── whatsapp.repository.ts # Prisma queries for WhatsAppTemplate/WhatsAppTemplateUsage/WhatsAppServiceConversation models: create, groupBy, raw SQL for DATE_TRUNC daily aggregation y UNION ALL para avg cost per request
     ...
     │   │   │   │   # PromptTemplates: parse_datetime_natural (vars: now, nextDays, input), classify_description (vars: categoryName, description), extract_name (vars: input) — AUT-494. compare_addresses (vars: addressA, addressB), extract_service_and_zone (vars: message), user_requests_license (vars: description) — AUT-499
   │   │   │   ├── matching/
...
│   │   │   │   ├── requests.controller.ts # Request validation, response formatting. confirmSchedule: cuando el profesional propone otro horario desde el panel web (AWAITING_USER_CONFIRMATION + proposedAt), tras el upsert de BotSession invoca notificationService.notifyUserAlternativeSchedule para avisar al usuario proactivamente por WhatsApp; se eliminó pendingMessage del tempData ya que el mensaje se envía directamente — AUT-492
│   │   │   │   ├── requests.service.ts    # Request lifecycle, matching, reassignment, timeouts (cron log + CREATED→NO_RESPONSE notification, AUT-287) + technicalBrief support + análisis LLM síncrono en create() via promptService.getPrompt('classify_description') (AUT-494, antes AUT-251) + persistencia de barrio y código postal desde Nominatim (AUT-307) + pasa userId en los 6 llamados a findBestCandidate (AUT-366) + notifyProfessionalReassigned en reject y reassignAfterNegotiation: el nuevo profesional recibe notificación WhatsApp inmediata al ser reasignado (AUT-425) + create() usa findActivesByUserId para validar por categoría en vez del método singular: bloquea solo si ya hay un pedido activo de la misma categoría (AUT-466)
│   │   │   │   └── requests.repository.ts # Prisma queries for Request/RequestEvent/Feedback + CreateRequestInput con technicalBrief + findActivesByUserId: devuelve todos los pedidos activos incluyendo category y geoNode (AUT-466) + findActivesByProfessionalId (AUT-298) + findConflictingSchedule filtra por status ASSIGNED/ACCEPTED para evitar que pedidos cancelados/completados bloqueen horario del profesional (AUT-433)
│   │   │   ├── reputation/
...
│   │   │   └── notifications/              # (AUT-195, AUT-199, AUT-295, AUT-297, AUT-301, AUT-343, AUT-392, AUT-428, AUT-439, AUT-442, AUT-469, AUT-492)
│   │   │       └── notification.service.ts   # WhatsApp dispatch with smart template/text strategy. Photos/audio deferred: sent only when professional asks for details via CoordinationService.sendRequestMedia (AUT-281). Description mismatch notification: notifyUserDescriptionMismatch + notifyUserConfirmService (AUT-295). CONFIRM_SERVICE context separado de DESCRIPTION_MISMATCH para resolver opciones correctamente según _mismatchType (AUT-393). Cancel confirmation: notifyUserCancelConfirmation (AUT-297). notifyProfessionalWithDetails usa TEMPLATE_PRO_NUEVO_PEDIDO_SIN_MEDIA con botones Aceptar/Ahora no puedo cuando no hay media (AUT-301). notifyUserRequestAccepted incluye badge "✓ Profesional matriculado" cuando aplica (AUT-343). notifyProfessionalPositiveFeedback: mensaje de felicitación cuando el usuario califica con wouldRecommend=true y rating>=4 (AUT-367). notifyProfessionalWithDetails y notifyProfessionalReminder escriben datos de la sesión en ProfessionalRequestSession (PRS) y actualizan BotSession.currentFlow — AUT-442. notifyProfessionalReminder incluye identificador "📋 [categoría] en [zona]" para soportar pedidos simultáneos — AUT-439. notifyUserNoResponse hace upsert de sesión del usuario a USER_REQUEST/WAITING_CONSENT después de enviar el mensaje, para que cualquier sesión previa quede pisada correctamente (AUT-428). notifyUserNoResponse acepta categoryName opcional como parámetro y lo pasa al template nora_user_sin_profesional como {{1}} para identificar el pedido en mensajes de "sin profesional" (AUT-469). notifyUserAlternativeSchedule: notifica proactivamente al usuario por WhatsApp cuando el profesional propone otro horario desde el panel web, usando sendWithWindowCheck con template nora_user_horario_alternativo (3 params: professionalName, categoryName, alternativeText) y quick replies Sí/No — replica el comportamiento de CoordinationService.confirmVisit para el flujo del panel web (AUT-492)
│   │   │   └── storage/
...
│   │   │   │   ├── bot.service.ts        # Message processing, flow dispatch, session management, pending notifications, cancellation detection with LLM fallback for USER and PROFESSIONAL roles (AUT-169, AUT-296, AUT-298), ver_como_funciona handler (AUT-266) + intercepción inteligente de servicio/zona en mensaje inicial (AUT-308) + extracción de nombre en paralelo con servicio/zona para usuarios nuevos (AUT-309) + POST_CANCEL flow en cancelación sin Request activo (AUT-310) + capa de seguridad (rate limiting + anti prompt injection) como primera línea de processMessage (AUT-312) + pre-filtro determinístico hasActionableContent para evitar LLM calls en saludos simples (AUT-314) + extracción LLM para usuarios recurrentes sin flujo activo (AUT-317). shouldKeepRegisterSession y session upsert protegen ASK_LICENSE_EARLY: no pisan la sesión ni bloquean al profesional durante la pregunta de credencial — AUT-323 + detección por keywords de intención de acceder al panel con respuesta de link directo vía session token (AUT-339) + resolveProfessionalState: control de frecuencia (máx 1 respuesta por día) y LLM contextual para ACTIVE/OBSERVATION fuera de flujo (AUT-360) + respuesta cordial para usuario sin sesión ni flujo activo cuando hasActionableContent es false, sin invocar LLM (AUT-363) + propaga buttonPayload al FlowContext para que los handlers procesen respuestas de botones interactivos (AUT-382) + extracción de servicio/zona en sesión sin currentFlow para detectar contexto en mensajes subsiguientes (AUT-389) + saludo diferenciado usuario nuevo vs recurrente en mensajes sin contenido accionable (AUT-389) + ACK silencing: inverted logic using INFORMATIONAL_STEPS + stepIsInformational() — defaults to letting messages through (safe), only silences on explicitly informational steps where Nora notifies without asking (AUT-502) + lectura/escritura de ProfessionalRequestSession para pedidos simultáneos del profesional: inyecta PRS antes del flow handler, escribe PRS después, usa PRS en pendingNotification y asegura PRS en resolveProfessionalState — AUT-443 + interceptor de cancelación se dispara para PROFESSIONAL aunque currentFlow sea null (condición `session.currentFlow || role === 'PROFESSIONAL'`), permitiendo cancelar pedidos ACCEPTED+SCHEDULED con sesión limpia — AUT-455 + los steps CANCEL_CONFIRMATION y SELECT_CANCEL_REQUEST con currentFlow null quedan excluidos de resolveProfessionalState y del bloque de inyección de PRS, y se fuerza currentFlow a COORDINATION en memoria (sin upsert a DB) antes del flowHandler, para que la confirmación de cancelación del profesional con sesión limpia ejecute la cancelación en vez de caer en AWAITING_AVAILABILITY — AUT-461 + soporte de múltiples pedidos activos por usuario con UserRequestSession: creación de URS al confirmar pedido, eliminación al cancelar/completar, listado de pedidos activos cuando el usuario escribe sin currentFlow, handlers SELECT_ACTIVE_REQUEST y SELECT_CANCEL_USER_REQUEST, cancelación multi-pedido del usuario usando findActiveUserRequestSessions — AUT-467 + actualización de UserRequestSession con finalTempData completo en cada cambio de step del usuario para persistir professionalName/scheduledAt cuando se coordina visita (AUT-471) + interceptor AWAITING_VISIT + COORDINATION: detecta usuarios con visita coordinada y ofrece menú de nuevo pedido en vez de bloquear la interacción, con step SELECT_NEW_REQUEST_CONFIRM (AUT-471) + selector de "No quiero cancelar ninguno" via numero en fallback (ya presente) + exclusion de SELECT_CANCEL_USER_REQUEST del bloque SELECT_ACTIVE_REQUEST para evitar override + extraccion de servicio/zona en sesion USER_REQUEST/INIT para nuevo pedido desde menu de visitas coordinadas — AUT-479 + fix: al construir los listados de pedidos (cancelación y selección de pedido activo) y al persistir en _activeUserSessions, la zona se lee con fallback (data.geoNodeName || data.zoneName) porque el flow de creación de pedido guarda la zona como zoneName y el de coordinación como geoNodeName; sin el fallback la zona nunca aparecía en el listado — AUT-483 + fix: el interceptor de cancelación ya no se dispara para usuarios sin flujo activo (condición `session.currentFlow || role === 'PROFESSIONAL'`, se eliminó `|| role === 'USER'`), evitando que un usuario sin currentFlow quede atrapado en SELECT_CANCEL_USER_REQUEST; al elegir "No quiero cancelar ninguno" (por número o por texto) la sesión se restaura al estado previo real (`_previousFlow`/`_previousStep` o null) en vez de hardcodear COORDINATION/AWAITING_VISIT — AUT-484 + fix: el handler de SELECT_ACTIVE_REQUEST ahora guarda currentStep SELECT_REQUEST_ACTION (antes WAITING) y arma el menú "Ver estado / Cancelar" con categoryName y clientAddress del tempData completo de la UserRequestSession, evitando que el siguiente mensaje caiga en handleWaiting y marque NO_RESPONSE un pedido ya coordinado — AUT-485 + con múltiples UserRequestSession activas, si el texto tiene contenido accionable (intención de nuevo pedido) arranca USER_REQUEST desde INIT preservando sessionTempData con _extractedServiceName/_extractedZoneName en vez de mostrar SELECT_ACTIVE_REQUEST; el listado de pedidos activos solo se muestra ante texto sin contenido accionable (saludos) — AUT-487 + fallback del handler SELECT_ACTIVE_REQUEST: si el input no es número válido y tiene contenido accionable arranca USER_REQUEST desde INIT sin return (continúa al dispatch); si no, muestra el listado completo con zona/dirección y opción "Hacer un nuevo pedido". El cast de _activeUserSessions incluye geoNodeName y clientAddress — AUT-489
│   │   │   │   ├── bot.repository.ts     # Prisma queries for BotSession model + wasTemplateSentInLast24h/setLastTemplateSentAt (AUT-272). setLastTemplateSentAt usa upsert en lugar de update para evitar P2025 cuando no existe sesión previa — AUT-401. Métodos ProfessionalRequestSession: upsertRequestSession, findActiveRequestSessions, findRequestSessionByRequestId, deleteRequestSession — AUT-442. Métodos UserRequestSession: upsertUserRequestSession, findActiveUserRequestSessions, findUserRequestSessionByRequestId, deleteUserRequestSession — AUT-465. findActiveUserRequestSessions filtra currentStep: { not: null } para excluir pedidos terminados (NOT_FULFILLED / handleWaiting sin consenso) del listado de pedidos activos del usuario, conservándolos como dato estadístico — AUT-488
...
│   │   │   │   ├── coordination.service.ts # Visit coordination relay: init after accept, send reminders, work-completion checks, confirmVisit usa request.scheduledAt directamente, sin LLM (AUT-248, AUT-474) y GPS opcional cuando userLatitude es null (AUT-315), notifyProfessionalVisitConfirmed envía directamente al profesional con sendWithWindowCheck usando templates nora_pro_visita_confirmada o nora_pro_visita_confirmada_ubicacion según corresponda (AUT-380). notifyProfessionalVisitConfirmed y notifyProfessionalClientAcceptedSchedule incluyen identificador "📋 [categoría] en [zona]" opcional para pedidos simultáneos — AUT-439. notifyProfessionalVisitConfirmed acepta securityCode y scheduledAt opcionales: cuando la visita queda coordinada en menos de 20hs (el recordatorio 20-24hs no se dispara) incluye el código de seguridad en el mensaje al profesional — AUT-453. notifyProfessionalVisitConfirmed también solicita la foto de perfil: si el profesional no tiene ProfessionalProfile.photoUrl, agrega el pedido de foto al mensaje y setea ProfessionalProfile.photoRequestedAt (upsert) como señal persistente para que la imagen enviada en AWAITING_VISIT se guarde como foto de perfil — AUT-454. sendRequestMedia deprecado: el envío de fotos/audio en "Ver detalles" ahora lo maneja webhooks.routes.ts via mediaFirst (AUT-290). notifyWorkFinished simplificado: mensaje de rating 1-5 sin botones, step inicial FEEDBACK_RATING (AUT-318). notifyUserNegotiationExhausted: envía mensaje condicional al usuario cuando se agotan las rondas de negociación, con sendWithWindowCheck usando templates nora_user_reasignando_por_negociacion o nora_user_sin_profesional_negociacion (AUT-379). sendReminders: ventana 20-24h, restricción horaria 8-21 Argentina (getHoursArgentina/getMinutesArgentina), mínimo 6hs post-coordinación (updatedAt), anti-duplicados vía reminderSentAt (AUT-405). sendReminders ahora también hace upsert de sesión del usuario a COORDINATION/AWAITING_VISIT_CONFIRMATION con requestId, userId, professionalId, professionalPhone y scheduledAt, para que el usuario pueda confirmar o cancelar desde el recordatorio (AUT-423). sendReminders incluye "📋 [categoría]" en recordatorio al profesional — AUT-439. checkWorkCompletion lee estado de ProfessionalRequestSession (PRS) en lugar de BotSession.tempData para retry logic; sendReminders profesional escribe en PRS — AUT-442. sendReminders y finalizeLocation incluyen código de seguridad de 4 dígitos y link al perfil público del profesional en el mensaje al usuario — AUT-450. El código y el link solo se incluyen si la visita es en menos de 20hs (AUT-451). confirmVisit: si el request ya tiene clientAddress saltea AWAITING_LOCATION y va directo a SCHEDULED notificando a ambas partes — AUT-472
...
│   │   │   │   ├── flows/
│   │   │   │   │   ├── types.ts          # Type definitions for flows. BotResponse incluye mediaFirst para control de orden de envío (AUT-290). SavedLocation interface para multi-dirección guardada (AUT-306). PendingNotification con templateName/templateParams opcionales para templates específicos (AUT-374). IncomingMessage.messageId para deduplicación de webhooks (AUT-394). IncomingMessage.mediaId/mediaType para descarga diferida de media con control de contexto (AUT-429)
│   │   │   │   │   ├── user-request.flow.ts        # USER_REQUEST flow: INIT → ASK_NAME → ASK_SERVICE → ASK_SAVED_LOCATION → ASK_LOCATION (GPS/Nominatim → department) → ASK_PROVINCE/ASK_ZONE (fallback) → ASK_DESCRIPTION → DESCRIPTION_MISMATCH → CLARIFICATION → ASK_PHOTOS → CONFIRM → WAITING → POST_CANCEL (AUT-310, AUT-386, AUT-387). Eliminado step SEARCHING: al confirmar con profesional asignado, no se envía mensaje adicional (AUT-387). Texto del CONFIRM actualizado para clarificar que NORA avisará cuando un profesional confirme (AUT-387). Eliminado step ASK_AUDIO: después de ASK_PHOTOS se va directo a CONFIRM vía buildConfirmation, ahorrando 1 mensaje saliente por pedido (AUT-386). La ubicación se resuelve justo después del servicio. Si el usuario comparte GPS, Nominatim resuelve departamento automáticamente. Si tiene savedLocations, se le ofrece reutilizar una dirección. Si no, cae al flujo de zona manual (AUT-306) + handlers para payloads de botones + generateClarificationQuestions (LLM, AUT-280, AUT-376) + generateTechnicalBrief (LLM, AUT-280) + validateDescription (LLM, 3 niveles: VALID/INVALID/UNCERTAIN, AUT-280, AUT-288) + validateClarificationAnswer (LLM, AUT-280) + handleDescriptionMismatch permite cambiar servicio o reformular (AUT-288) + proceedAfterService saltea zona si ya está definida (AUT-288) + extractName con LLM en handleAskName (AUT-301) + mensajes simplificados: "no" reemplaza "continuar"/"omitir" en fotos/audio/ubicación (AUT-301) + transcripción de audio en ASK_DESCRIPTION via transcribeAudio: si usuario manda audio se transcribe como descripción (AUT-302) + resolveExtractedServiceZone y handleAskService con extracción inteligente de servicio/zona del mensaje inicial: saltea pasos ya respondidos, notifica servicio/zona inexistentes (AUT-308) + handleInit con múltiples pedidos activos: findMany + valida por categoría — solo bloquea si ya existe un pedido de la misma categoría (CREATED/ASSIGNED/ACCEPTED/PENDING_CONFIRMATION/NO_RESPONSE con waitingUserConsent); mensaje de bloqueo incluye el nombre de la categoría (AUT-468). Cancel interceptor con findMany: 1 solo pedido → confirmación directa; múltiples → delegado a bot.service.ts (AUT-468) + handleInit con pedido activo: respuesta contextual por estado (CREATED/ASSIGNED/ACCEPTED) en vez de ASK_PHOTOS (AUT-363) + handleInit con bienvenida humanizada cuando se detecta servicio/zona en tempData para usuarios nuevos (AUT-389) + handleAskService: saludo "Qué bueno volver a verte" limitado a 1 vez por día vía lastGreetingAt en tempData (AUT-363) + proceedAfterService: humaniza mensajes de confirmación ("Entendido" → "¡Perfecto!"), filtra savedLocations por geoNodeId coincidente cuando zona ya fue detectada por LLM y solo ofrece direcciones de la misma zona (AUT-372) + handleClarification/handleAskPhotos: saltea pregunta de audio cuando la descripción por texto ya es completa según LLM, flag _descriptionIsComplete (AUT-375). buttonPayload en handlers de opciones (CONFIRM, POST_CANCEL, WAITING_CONSENT, DESCRIPTION_MISMATCH) + INIT payloads desde message.buttonPayload (AUT-382) + handleAskServiceList: mensaje de error humanizado con ejemplo dinámico cuando usuario escribe texto en lugar de número en selección de servicio (AUT-395) + interceptor de cancelación en handleStep antes del switch de steps: cuando step===INIT y el mensaje incluye alias de cancelación y existe pedido activo en CREATED/ASSIGNED/ACCEPTED, NORA pregunta confirmación en vez de responder con el estado (AUT-426) + interceptor de cancelación en handleStep: agrega options numeradas (Sí cancelar / No seguir con el pedido) y persiste categoryName en tempData — AUT-427 + handleConfirm path 2 (sin match y sin profesionales trial-exhausted): el pedido queda en NOT_FULFILLED (no NO_RESPONSE) con mensaje claro sin falsas expectativas, para que no aparezca en el listado de cancelación del usuario — AUT-480 + steps SELECT_REQUEST_ACTION y VIEW_REQUEST_STATUS: al seleccionar un pedido desde SELECT_ACTIVE_REQUEST, handleSelectRequestAction resuelve "Ver estado"/"Cancelar" con resolveOptionWithFallback; "Ver estado" encadena a handleViewRequestStatus que muestra profesional, fecha/hora de visita, dirección, código de seguridad y link de perfil (si el profesional tiene photoUrl) cuando el pedido tiene scheduledAt+professionalName, o mensaje de búsqueda si no; "Cancelar" deriva a CANCEL_CONFIRMATION. Elimina el bug donde handleWaiting se ejecutaba sobre pedidos ya coordinados marcándolos NO_RESPONSE — AUT-485 + handleCancelarVisita y handleCancelarPedido usan tempData.requestId con findUnique en vez de buscar por userId con findFirst: con múltiples pedidos activos ya no se cancela el pedido equivocado; si falta requestId devuelven mensaje de error claro sin crash — AUT-490
...
│   │   │   │   │   ├── coordination.flow.ts  # COORDINATION: visit scheduling relay flow (AUT-388: saltea confirmación cuando parseDateTimeNatural resuelve sin ambigüedad; AUT-247: confirmación antes de avanzar, mensajes sin ejemplos; AUT-248: mensajes diferenciados past/ambiguous, clientAvailability con fecha formateada; parseDateTimeNatural obtiene promptTemplate desde promptService.getPrompt('parse_datetime_natural'), eliminando prompt hardcodeado (AUT-494); AUT-290: mediaUrls/audioUrl/mediaFirst en handleAwaitingAcceptance en vez de sendRequestMedia; AUT-291: PROPOSE_ALTERNATIVE en handleAwaitingConfirmation; AUT-294: off-topic detection en 6 steps con resolveOptionWithFallback; AUT-301: handleAwaitingAcceptance sin media muestra Aceptar/Ahora no puedo directo sin "Ver detalles"; AUT-422: bug fix — '1' en primer intercambio sin media acepta directamente sin pasar por VER_DETALLES; AUT-436: buildAcceptanceText envía link del panel web al aceptar el primer pedido, rastreado vía ProfessionalEvent PANEL_INTRO_ACCEPTANCE, con try/catch para que el flujo continúe sin link si generateSessionToken falla. AUT-448: buildAcceptanceText notifica al profesional que suba su foto de perfil si ProfessionalProfile.photoUrl no existe. AUT-454: se eliminó ese tip de foto de buildAcceptanceText — queda reemplazado por el pedido de foto en notifyProfessionalVisitConfirmed. handleAwaitingVisit ahora procesa una imagen enviada por el profesional en AWAITING_VISIT: hace upsert de ProfessionalProfile con photoUrl y limpia photoRequestedAt a null, respondiendo con confirmación (AUT-454). AUT-451: finalizeLocation solo incluye código de seguridad y link al perfil si la visita es en menos de 20hs. AUT-453: los 4 calls a notifyProfessionalVisitConfirmed pasan securityCode y scheduledAt para que el profesional reciba el código cuando la visita es en menos de 20hs; en los 2 calls que dependen de finalizeLocation el securityCode se genera antes y se propaga vía _securityCode). AWAITING_GPS: paso previo opcional cuando userLatitude es null, guarda coords y barrio vía Nominatim async, chequea clientAddress pre-llenado antes de pedir dirección (AUT-317), avanza a AWAITING_LOCATION (AUT-315). AWAITING_LOCATION: savedLocations con sugerencias por geoNodeId, comparación LLM de direcciones (compareAddresses), upsert con límite configurable USER_MAX_SAVED_LOCATIONS, pre-llenado de clientAddress desde savedLocation reutilizada (AUT-306). AWAITING_LOCATION: extractCleanAddress limpia frases introductorias del texto de dirección con LLM antes de guardar en savedLocations, con fallback al texto original si el LLM falla (AUT-381). notifyProfessionalVisitConfirmed se llama con await + professionalPhone en 2 instancias (solo cuando clientAddress existe, y ahora también en handleAwaitingUserConfirmation cuando el usuario acepta horario alternativo — AUT-402). Bug fix AUT-417: _confirmedByProfessional corregido a true en handleAwaitingUserConfirmation para evitar doble notificación al profesional. finalizeLocation y handleAwaitingUserConfirmation generan código de seguridad de 4 dígitos al quedar SCHEDULED, persisten en Request.securityCode y consultan ProfessionalProfile.photoUrl para incluir link al perfil público del profesional en el mensaje al usuario — AUT-450 cuando clientAddress ya existe. Bug fix AUT-434: flag _userAcceptedAlternative en handleAwaitingUserConfirmation y finalizeLocation para mostrar el saludo correcto cuando el usuario acepta horario alternativo del profesional. handleAwaitingAvailability: guard de rol PROFESSIONAL para evitar que el profesional reciba mensaje pensado para el usuario cuando su sesión cae en este step (AUT-403). response.text vacío porque el envío lo hace el método directamente con templates específicos (AUT-380). Eliminadas las 2 llamadas que pasaban zoneName como dirección cuando clientAddress aún no estaba confirmada, ya que finalizeLocation cubre ese caso correctamente con la dirección real (AUT-383). finalizeLocation: mensaje condicional explícito cuando el profesional confirma propuesta de horario del usuario vía flag _confirmedByProfessional (AUT-373). pendingNotification: usa templateName/templateParams específicos por caso (5 casos) en vez del genérico nora_notification (AUT-374). POST_NEGOTIATION: step posterior al agotamiento de rondas de negociación, maneja respuesta del usuario (seguir esperando / cancelar pedido) con mensajes condicionales según resultado de reassignAfterNegotiation (AUT-379). buttonPayload en handlers de opciones (AWAITING_ACCEPTANCE, CONFIRM_AVAILABILITY, CONFIRM_PRO_AVAILABILITY, POST_NEGOTIATION, AWAITING_VISIT_CONFIRMATION): usa buttonPayload || text para inputText (AUT-382). finalizeLocation notifica al profesional con dirección + teléfono + link de mapa vía notifyProfessionalVisitConfirmed solo cuando _confirmedByProfessional no es true, evitando doble envío en path con clientAddress existente (AUT-414, AUT-415). handleAwaitingVisitConfirmation: bug fix AUT-423 — reemplaza guard role !== PROFESSIONAL por bloque role === USER que procesa respuesta del usuario: CONFIRM vuelve a AWAITING_VISIT, CANCEL deriva a CANCEL_CONFIRMATION con cancelTarget:visit; bloque PROFESSIONAL conserva lógica original intacta. AUT-440: mensaje de propuesta de horario al profesional incluye identificador "📋 [categoría] en [zona]" con fallback a DB. AUT-456: unificación del mensaje al usuario cuando la visita queda coordinada — handleAwaitingUserConfirmation propaga scheduledAt al tempData de finalizeLocation, y finalizeLocation usa un único greeting ("✅ ¡Perfecto! Ya está todo coordinado con [nombre].") para ambos paths (profesional acepta horario del usuario / usuario acepta horario alternativo), incluyendo fecha, hora y código de seguridad si la visita es en menos de 20hs
│   │   │   │   │   ├── feedback.flow.ts      # FEEDBACK: work completion + bilateral rating flow + sentiment analysis (AUT-216, AUT-235). Flujo simplificado a 4 mensajes: satisfaction derivado automáticamente del rating (1-2 → UNSATISFIED, 3 → PARTIAL, 4-5 → SATISFIED), eliminando step FEEDBACK_SATISFACTION (AUT-318). Off-topic detection en FEEDBACK_RECOMMEND, FEEDBACK_PRO_RECOMMEND (AUT-294). Captura de precio pagado via FEEDBACK_AMOUNT step (AUT-316). NotificationService inyectado para notifyProfessionalPositiveFeedback: felicitación al profesional cuando el feedback del usuario es positivo (AUT-367). Template condicional en handleFeedbackRecommend según feedback: nora_pro_felicitacion_calificacion cuando wouldRecommend y rating >= 4, sino nora_pro_pedir_calificacion_usuario. Notificación al profesional disparada inmediatamente después de FEEDBACK_RECOMMEND, sin depender de pasos opcionales (precio/comentario) (AUT-370, AUT-371). Link del panel web al finalizar el primer trabajo, rastreado vía ProfessionalEvent PANEL_INTRO_COMPLETION, con try/catch para que el flujo continúe sin link si generateSessionToken falla (AUT-437). ProfessionalsService inyectado en el constructor.
│   │   │   │   │   ├── ack-detector.helper.ts   # Detector de mensajes ACK (gracias, ok, 👍) + INFORMATIONAL_STEPS y stepIsInformational() para lógica invertida de pasos donde Nora solo notifica sin esperar input (AUT-397, AUT-502)
│   │   │   │   │   ├── cancel-flow.helper.ts  # Shared cancellation confirmation logic (AUT-296: avoids re-entry loop + friendly error on duplicate cancel; AUT-297: numbered options + sendWithWindowCheck via NotificationService for template fallback; AUT-298: LLM-based cancellation intent detection + professional role support in handleCancelConfirmation; AUT-462: cancelación por profesional — responseText fijo para el profesional ("Entendido. Cancelaste la visita...") en vez del userMessage, y pendingNotification al usuario ahora setea templateName (nora_user_pro_cancelo_visita si había visita coordinada, nora_user_pro_cancelo_pedido si no) y templateParams para que el fallback fuera de la ventana 24hs use el template correcto en vez de fallar silenciosamente)
...
│   │   │   ├── admin-api.ts           # REST client for all admin endpoints. adminCreateProfessional: alta directa de profesional con disponibilidad opcional — AUT-351, AUT-354. updateProfessional, uploadProfessionalFile: edición de profesional con upload de archivos — AUT-355. getMembershipDiscountConfig, setMembershipDiscount — AUT-334. getCurrentPlan: consulta de plan activo — AUT-348. cancelMembership: cancelación de membresía — AUT-353. getPendingDataChanges: incluye previousValues y newValues — AUT-359. getProfessionalsWithPendingChanges: IDs de profesionales con cambios pendientes — AUT-359. getPrompts, updatePrompt, resetPrompt — AUT-400. getDemandInsights: consulta de demanda insatisfecha detallada (GET /admin/demand) — AUT-482. getLLMCosts: costos LLM agregados con filtro opcional from/to — AUT-496. getWhatsAppCosts, getWhatsAppTemplates, updateWhatsAppTemplate: costos WhatsApp y gestión de catálogo de templates — AUT-498
...
│   │   │   ├── panel-api.ts           # REST client for professional panel. updateProfile (PATCH), uploadProfileFile (R2 presigned), uploadProfilePhoto (PATCH profile/photo) — AUT-447, getDepartments (session-authenticated) — AUT-356
...
│   │   │   ├── admin.ts               # Interfaces for all admin entities (Professional, User, Request, Escalation, etc.) + MembershipStatusKind extendido con CANCELED — AUT-353 + PromptTemplate para gestión de prompts LLM — AUT-400 + DemandInsights (byCategory con lastDate, total, totalCategories) — AUT-482 + AggregatedLLMUsage + LLMCosts (costos LLM agregados) — AUT-496 + WhatsAppTemplateUsage + WhatsAppCosts + WhatsAppTemplate (costos WhatsApp) — AUT-498
...
│   │   │   │   ├── DemandPage.tsx              # Página admin dedicada de demanda sin cobertura (ruta /demand): cards resumen (total NOT_FULFILLED + categorías afectadas), gráfico de barras top 10 categorías/zonas (naranja), tabla detallada CATEGORÍA/ZONA/PEDIDOS/ÚLTIMA SOLICITUD ordenada por pedidos desc, estado vacío. Consume getDemandInsights de admin-api — AUT-482
...
│   │   │   │   ├── LLMCostsPage.tsx              # Página admin de costos LLM (ruta /admin/llm-costs, SUPERADMIN): filtro de fechas, 4 cards resumen (costo total, promedio por pedido, total llamadas, proyección mensual), gráfico de tendencia diaria (LineChart con doble eje Y: costo USD + llamadas), breakdown asociadas vs generales con porcentajes, tabla por prompt key ordenada por costo descendente con badges de color, tabla por modelo con % del total — AUT-496
     │   │   │   │   └── WhatsAppCostsPage.tsx          # Página admin de costos WhatsApp (ruta /admin/whatsapp-costs, SUPERADMIN): filtro de fechas (default 30d), 5 cards resumen (templates enviados, costo templates, conversaciones servicio, costo servicio, costo promedio por pedido), gráfico de tendencia diaria (LineChart templates vs servicio), resumen por categoría (utility vs marketing), tabla de templates con edición inline de categoría (sin columna de costo unitario) — AUT-498, AUT-501
│   │   │   │   └── WhatsAppPricingPage.tsx        # Página admin de precios WhatsApp (ruta /admin/whatsapp-pricing, SUPERADMIN): 4 cards de precios por categoría de conversación editables (Servicio, Utilidad, Marketing, Autenticación) que persisten en SystemConfig, tabla de templates con edición inline de categoría (sin columna de costo individual, ya que el costo es por categoría) — AUT-500, AUT-501
...
│   │   │   └── public/
│   │   │       └── ProfessionalPublicPage.tsx # Página pública del perfil del profesional: mobile-first, foto/nombre/categoría/badges (verificado/excelencia), sin datos de contacto — AUT-449
...
│   │   │           ├── ProfessionalProfile.tsx    # Status badge (px-4 py-1.5 text-sm), excellence badge (text-sm), availability, personal data (py-4 rows, text-base font-semibold values), docs (read-only), sección Foto de perfil con banner de alerta (sin foto), placeholder circular, upload a R2 vía uploadProfileFile — AUT-447, botón "Editar perfil" con prop onEdit — AUT-356, max-w-5xl (AUT-181, AUT-183)
...
│   ├── webhooks.routes.ts      # WhatsApp webhook endpoint (AUT-134), photo debounce accumulation (AUT-283), sendResponse con soporte mediaFirst: fotos/audio antes que texto con delay de 1500ms para orden visual (AUT-290, AUT-317), propaga buttonPayload a processMessage en ambos llamados (AUT-382), Redis SET NX para deduplicación de webhooks por messageId con TTL 1h (AUT-394). MEDIA_ALLOWED_STEPS: mapa de steps que aceptan media; descarga condicional en processWebhookAsync con respuesta clara al usuario si media fuera de contexto (AUT-429). Captura de foto de perfil: si un profesional envía una imagen y tiene ProfessionalProfile.photoRequestedAt pendiente sin photoUrl, la imagen se descarga a la carpeta R2 'profile-photos' y se procesa por processMessage (bypass de MEDIA_ALLOWED_STEPS y del debounce de fotos) — AUT-454. La condición ya no depende del step AWAITING_VISIT: el profesional puede mandar la foto en cualquier momento después de que NORA se la pidió (photoRequestedAt es la única señal necesaria), evitando el error cuando su sesión ya está limpia (INIT) — AUT-460
...
Registro del uso y costo de cada llamada LLM para cálculo de costo operativo por pedido y proyecciones de negocio.
...
|---------------|----------|----------------------------------------------------------------|
| `id`           | String   | PK (cuid)                                                      |
| `requestId`    | String?  | FK → Request (nullable)                                        |
| `userId`       | String?  | FK → User (nullable)                                           |
| `promptKey`    | String?  | Identificador del prompt (ej: `classify_description`)          |
| `provider`     | String   | Proveedor LLM: `openai` o `anthropic`                         |
| `model`        | String   | Modelo utilizado (ej: `gpt-4o-mini`, `claude-haiku-4-5-20251001`) |
| `inputTokens`  | Int      | Tokens de entrada consumidos                                   |
| `outputTokens` | Int      | Tokens de salida generados                                     |
| `costUsd`      | Float    | Costo estimado en USD                                          |
| `durationMs`   | Int?     | Duración de la llamada en milisegundos                         |
| `createdAt`    | DateTime | Fecha de creación                                              |

**Índices:** `@@index([createdAt])`, `@@index([promptKey])`, `@@index([model])`, `@@index([requestId])`, `@@index([userId])`

**Relaciones inversas:** `Request.llmUsages[]`, `User.llmUsages[]`

**Endpoint:** `GET /admin/llm-costs?from=&to=` devuelve costos agregados por modelo, promptKey, provider, costo total, promedio por pedido, tendencia diaria y breakdown asociadas vs generales.
...
**Nota (AUT-501):** `costUsd` fue eliminado de `WhatsAppTemplate`. El costo por categoría ahora se configura en `SystemConfig` con las keys:
- `WHATSAPP_SERVICE_CONVERSATION_COST_USD`
- `WHATSAPP_UTILITY_CONVERSATION_COST_USD`
- `WHATSAPP_MARKETING_CONVERSATION_COST_USD`
- `WHATSAPP_AUTHENTICATION_CONVERSATION_COST_USD`

Al registrar un envío de template, el `costUsd` se calcula automáticamente según la categoría del template y el precio configurado en `SystemConfig`. Las conversaciones de servicio usan `WHATSAPP_SERVICE_CONVERSATION_COST_USD`.
...
| `REDIS_URL` | — | URL de conexión a Redis para cache/deduplicación (AUT-394) |
...
| Flow                    | Estados                                                                 |
|------------------------|-------------------------------------------------------------------------|
| `USER_REQUEST`         | INIT → ASK_NAME → ASK_SERVICE → ASK_PROVINCE → ASK_ZONE → ASK_DESCRIPTION → CLARIFICATION → ASK_LOCATION → ASK_PHOTOS → CONFIRM → SEARCHING |
...
**Lógica de paso ASK_PHOTOS (AUT-280, AUT-386):** Acepta hasta 3 fotos. Al recibir la tercera, avanza automáticamente a `CONFIRM`. Cualquier texto distinto de "continuar" también avanza a `CONFIRM`. Eliminado step ASK_AUDIO (AUT-386).

**Lógica de paso ASK_AUDIO (AUT-280):** Eliminado en AUT-386.
...
- **`confirmSchedule` (service) — estados válidos de coordinación (AUT-491)**: la validación acepta `AWAITING_CONFIRMATION`, `AWAITING_USER_CONFIRMATION` y `AWAITING_AVAILABILITY`. Se agregó `AWAITING_AVAILABILITY` para permitir que el profesional contra-proponga un horario desde el panel web cuando el usuario ya propuso su disponibilidad (`scheduledAt` con valor). Antes rechazaba con 400 "Cannot confirm schedule when coordination is AWAITING_AVAILABILITY", aunque el flujo por WhatsApp sí lo permitía.
...
- Pedidos:
   - Usuario con pedido activo de la misma categoría (CREATED, ASSIGNED, ACCEPTED, PENDING_CONFIRMATION, NO_RESPONSE con waitingUserConsent) no puede crear otro pedido de esa categoría. Pedidos de categorías distintas se permiten — validado en `requests.service.ts:create()` (AUT-466) y en `user-request.flow.ts:handleInit()` (AUT-468)
...
- **Coordinación de visita (AUT-151, AUT-152, AUT-160, AUT-195, AUT-201):**
  ...
  - NORA actúa como relay entre usuario y profesional para coordinar día, hora y dirección exacta
   ...
   - **Intercepción AWAITING_VISIT con visita coordinada (AUT-471):** Cuando un usuario está en `COORDINATION/AWAITING_VISIT` y tiene visitas ya coordinadas (`professionalName` y `scheduledAt` en `UserRequestSession.tempData`), NORA muestra un menú con las visitas y ofrece iniciar un nuevo pedido de otro servicio. Si elige "Sí", limpia el flow y pregunta qué servicio necesita. Si elige "No", vuelve a `AWAITING_VISIT`. Si escribe "cancelar", el interceptor de cancelación toma precedencia.
   - `UserRequestSession` se actualiza con `finalTempData` completo en cada cambio de step del usuario (cuando `role === 'USER'` y `result.nextStep`). Esto asegura que datos como `professionalName` y `scheduledAt` se persisten cuando la visita queda coordinada (AUT-471).
   - **Eliminación de `UserRequestSession` al finalizar (AUT-486):** Cuando un flow de usuario devuelve `nextStep: null`, `bot.service.ts` elimina la `UserRequestSession` del pedido (asume ciclo finalizado: cancelación, etc.). Sin embargo, `handleViewRequestStatus` ("Ver estado") también devuelve `nextStep: null` sin que el pedido haya finalizado. Para evitar que se elimine la sesión del pedido consultado, ese handler marca `_skipSessionDelete: true` en su `tempData`, y el bloque de delete respeta ese flag (`if (requestId && !result.tempData._skipSessionDelete)`). Así el pedido sigue apareciendo en `SELECT_ACTIVE_REQUEST` tras consultar su estado, mientras que la cancelación (sin ese flag) sigue eliminando la sesión normalmente.
